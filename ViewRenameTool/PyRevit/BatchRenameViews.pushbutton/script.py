# -*- coding: utf-8 -*-
"""
Tool Đổi Tên Hàng Loạt View trong Revit
Batch Rename Selected Views with Level Prefix, Base Name, and Suffix
...
def get_view_level_name(view):
    ...
    if param and param.HasValue:
        ...
        if level_id and level_id != ElementId.InvalidElementId:
            ...
            if level_elem:
                return level_elem.Name
                
    # 3. BuiltInParameter VIEW_ASSOCIATED_LEVEL
    param_assoc = view.get_Parameter(BuiltInParameter.VIEW_ASSOCIATED_LEVEL)
...
def main():
    ...
    if not views:
        forms.alert("Không có View nào được chọn để đổi tên!", title="Thông báo")
        return

    items = []
    for v in views:
        lvl_name = get_view_level_name(v)
        items.append(ViewRenameItem(v, lvl_name))
    ...
    t = Transaction(doc, "View Rename - Antigravity")
    t.Start()
